...
//		}
	...
	public static HttpResponse updateWebSocketProtocol(HttpRequest request, ChannelContext channelContext) {
		...
		if (StringUtils.isNotBlank(Sec_WebSocket_Key)) {
			...
			byte[] key_array = SecureUtil.sha1(Sec_WebSocket_Key_Magic).getBytes();
			String acceptKey = Base64.getEncoder().encodeToString(key_array);
			...
			//httpResponse.addHeader(respHeaders);
			return httpResponse;
		}
		return null;
	}
